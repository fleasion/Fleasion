...
def _download_prejson_payload() -> None:
    ...
    for game_name, game_config in games.items():
        ...
        if github_url:
            try:
                ...
                log_buffer.log('PreJsons', f'Failed original {game_name}: {exc}')
        ...
        if replacement_url:
            try:
                ...
                log_buffer.log('PreJsons', f'Failed replacement {game_name}: {exc}')
...
def download_prejsons() -> None:
    ...
    try:
        ...
        _download_prejson_payload()
    except (urllib.error.URLError, json.JSONDecodeError, OSError) as exc:
        log_buffer.log('PreJsons', f'Failed to fetch CLOG.json: {exc}')
