...
def test_proxy_action_buttons_share_native_height() -> None:
    app = _qapp()
    ...
    for index in range(tab.ui.horizontalLayout_3.count()):
        item = tab.ui.horizontalLayout_3.itemAt(index)
    ...
    assert {tab.ui.clearButton, tab.ui.autoReplace, tab.ui.forwardButton, tab.ui.dropButton} < set(
        buttons
    )
