...
def test_target_launcher_uses_environment_proxy_on_linux(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        'fleasion.utils.platform_linux.relaunch_roblox_with_proxy_env',
        lambda proxy_url, target: calls.append((proxy_url, target)) or True,
    )

    assert roblox_target_launcher.launch_roblox_target(proxy, 'roblox-player:ticket')

    assert calls == [('http://127.0.0.1:8123', 'roblox-player:ticket')]
